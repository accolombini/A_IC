"""
Neste laboratório vamos extender os conceitos de retorno sobre o investimento,
considerando um portfólio de ações.
        NOTA:   o ponto aqui é encontrar o ótimo para sua carteira se tornar equilibrada e
                com retorno máximo
                Nosso portfólio será composto por:  PETR4.SA -> PETROBRAS
                                                    VALE3.SA -> VALE
                                                    CRFB3.SA -> CARREFOUR
                                                    TAEE3.SA -> TAESA
                                                    ^BVSP    -> ÍNDICE BOVESPA
...
tickers = ['PETR4.SA', 'VALE3.SA', 'CRFB3.SA', 'TAEE3.SA', '^BVSP']  # <class 'list'>
print(f'O tipo de tickers é -> {type(tickers)}')

mydata = pd.DataFrame()  # -> PETR4.SA VALE3.SA CRFB3.SA TAEE3.SA ^BVSP | valores que extrair
...
# A normalização será feita usando Pl / P0 * 100 => preço no dia 3/1/2000 => fixo =>> iloc[0]
...
weights_2 = np.array([0.4, 0.4, 0.10, 0.05, 0.05])
